...
|---|---|---|---|
| ~~**Falsificación de confirmación de pago PayU**~~ — **RESUELTO 2026-07-07** (tarea `fix_payu_signature_verification`, `tasks.json` id 26; ver `progress/history.md`). `updateUserStatus()` marcaba una inscripción como pagada solo verificando `polTransactionState == '4'` en el request; nunca validaba la firma (`sign`) entrante. La ruta (`POST /pagos/{id}/confirmation`) no tiene auth y está explícitamente eximida de CSRF. Fix: `App\Payments\Concerns\VerifiesPayuSignature` + rechazo 403 en `PagosController::confirmation()`. Pendiente: correr `phpunit` real y mergear la branch `claude/etapa0-payu-signature` (no se pudo verificar por ejecución en el sandbox que hizo el fix). | `app/Payments/PayU.php:136-145`, `app/Http/Controllers/PagosController.php:40-60`, `app/Http/Middleware/VerifyCsrfToken.php:15` (verificado por mí, código leído completo) | ~~**CRÍTICA**~~ → mitigada, pendiente de verificación por ejecución | Usuarios (fraude directo) + Seguridad + Mantenimiento (contradice el estándar de verificación de firma que sí se aplica correctamente en Stripe) |
| **IDOR en lectura de perfil**: `show(Persona $persona)` devuelve el objeto completo sin comparar contra el usuario autenticado. Iterando `idPersona` cualquier usuario con token lee mail, DNI, teléfono, fecha de nacimiento y género de cualquier otro. | `app/Http/Controllers/api/PersonasController.php:24-27` (verificado por mí) | **CRÍTICA** | Usuarios (fuga masiva de PII) + Seguridad |
| **IDOR en escritura de perfil**: `update(Request $request, Persona $persona)` no verifica ownership. Cualquier usuario autenticado puede sobrescribir el mail de cualquier otro — vector directo hacia account takeover vía reset de contraseña. | `app/Http/Controllers/api/PersonasController.php:244-281` (verificado por mí) | **CRÍTICA** | Usuarios + Seguridad (el más grave de los tres: no es solo lectura, es escritura de identidad ajena) |
