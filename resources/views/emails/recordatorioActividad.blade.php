    ...
    @if($inscripcion->actividad->show_dates || $inscripcion->actividad->show_location)
        <p style="margin:0; font-size:15px; line-height:1.5; color:#2b2f36;">
            ...
            @if($inscripcion->actividad->show_location)
                ...
                <strong>@if($inscripcion->actividad->idLocalidad){{$inscripcion->actividad->localidad->localidad}}, @endif{{$inscripcion->actividad->provincia->provincia}}, {{$inscripcion->actividad->pais->nombre}}</strong>
            @endif
        </p>
    @endif
